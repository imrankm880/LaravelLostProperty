...
class AdminItemsController extends Controller
{
    public function index()
    {
        ...
        $paginator = lostitems::with(['user', 'lostimage'])->paginate(10);


        return view('admin.itemsList', compact('paginator'));
    }
...
//             Report::with(['user', 'lostitem'])->where('status', 1)->update(['status' => 0]);
    ...
    function deleteImage($id){
        ...
        unlink(public_path() . '/img/items/' . $image->file_name);
        $image->delete();
        return redirect('dashboard/report-list/edit/' . $item_id);
    }
